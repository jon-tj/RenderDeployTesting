...
export class HubApi {
  ...
  updateEvent(
    id: number,
    payload: Partial<Pick<EventDetail, 'type' | 'title' | 'description' | 'location' | 'locationLabel' | 'dressCode' | 'startUtc' | 'endUtc' | 'mealOptions' | 'drinkOptions' | 'inheritParentInvites' | 'collectChildRsvps' | 'albumUploadPolicy' | 'showInviteesToGuests' | 'visibility' | 'enableTranslations'>> & { parentEventId?: number | null; translations?: Record<string, EventTranslation> },
  ): Promise<EventDetail> {
  ...
  importEvent(file: File, force = false): Promise<number> {
    const form = new FormData();
    form.append('file', file);
    const url = this.api.url(`/api/events/import${force ? '?force=true' : ''}`);
    return firstValueFrom(this.http.post<number>(url, form));
  }
